...
from helpers.constants import EMAIL_ADDRESS
...
def test_retrieving_email_with_image_attachment(clean_up):
    ...
    result = retrieve_email(EMAIL_ADDRESS, expected_subject)

    assert isinstance(result, Message)
    assert get_sender(result) == EMAIL_ADDRESS
...
def test_retrieving_email_from_invalid_mailbox():
    ...
    result = retrieve_email(EMAIL_ADDRESS, subject, mailxox)
...
def test_retrieving_email_with_non_existent_subject():
    ...
    result = retrieve_email(EMAIL_ADDRESS, subject)
